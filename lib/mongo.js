...
function hasTableMovement(table) {
  ...
  if (table.status && table.status !== 'livre' && table.status !== 'juntada') return true
  if ((table.items || []).length > 0) return true
  if (getTableTotal(table) > 0) return true
  if (Number(table.guests || table.peopleCount || 0) > 0) return true
  return Boolean(table.billRequested || table.kitchenSent || table.openedAt)
}
...
function mergeTablesPreservingMovement(incoming, existing) {
  const incomingByKey = new Map()
  const result = []

  for (const table of incoming) {
    const key = getDocumentKey('tables', table)
    if (!key) continue
    incomingByKey.set(key, table)
    result.push(table)
  }

  for (const rawTable of existing) {
    const table = stripMongoId(rawTable)
    const key = getDocumentKey('tables', table)
    if (!key || incomingByKey.has(key) || !hasTableMovement(table)) continue
    result.push(table)
  }
...
export async function syncCollectionState(db, collectionName, documents) {
  if (!Array.isArray(documents)) return null

  const collection = db.collection(collectionName)
  const existing = await collection.find({}).toArray()
  if (!documents.length && existing.length) return existing.map(stripMongoId)

  const finalDocuments = collectionName === 'tables'
    ? mergeTablesPreservingMovement(documents, existing)
    : documents
  ...
  const removableIds = existing
    .filter(document => {
      ...
      return collectionName !== 'tables' || !hasTableMovement(document)
    })
