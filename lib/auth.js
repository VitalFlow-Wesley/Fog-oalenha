...
// A manager password is only accepted at the authorization endpoint.  The
// resulting short-lived signed proof lets the state endpoint enforce that a
// waiter cannot remove an already launched item by crafting a direct request.
export function createManagerApproval(user) {
  const now = Math.floor(Date.now() / 1000)
  const payload = encode({ id: user.id, role: user.role, purpose: 'remove-item', iat: now, exp: now + APPROVAL_SECONDS })
  return `${payload}.${sign(payload)}`
}

export function readManagerApproval(token) {
  const [payload, signature] = String(token || '').split('.')
  if (!payload || !signature || !safeEqual(sign(payload), signature)) return null
  try {
    const approval = JSON.parse(Buffer.from(payload, 'base64url').toString('utf8'))
    return approval.purpose === 'remove-item' && ['admin', 'gerente'].includes(approval.role) && approval.exp > Math.floor(Date.now() / 1000)
      ? approval
      : null
  } catch {
    return null
...
function secureCookieAttribute() {
  return String(process.env.COOKIE_SECURE ?? 'true').toLowerCase() === 'false' ? '' : '; Secure'
}

export function setSessionCookie(res, token) {
  res.setHeader('Set-Cookie', `${COOKIE_NAME}=${encodeURIComponent(token)}; Path=/; HttpOnly${secureCookieAttribute()}; SameSite=Lax; Max-Age=${SESSION_SECONDS}`)
}

export function clearSessionCookie(res) {
  res.setHeader('Set-Cookie', `${COOKIE_NAME}=; Path=/; HttpOnly${secureCookieAttribute()}; SameSite=Lax; Max-Age=0`)
}
