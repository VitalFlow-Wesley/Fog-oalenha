...
export default function App() {
  ...
  async function handlePartialCloseTable(tableId, { items, guests, paymentMethod }) {
    ...
    const nextTable = {
      ...
      removedItemIds: (table.removedItemIds || []).concat(
        table.items.filter(item => !remainingItems.some(nextItem => (nextItem.lineId || `${nextItem.id}-${nextItem.observation || ''}-${nextItem.originTable || ''}-${nextItem.launchedByName || ''}`) === (item.lineId || `${item.id}-${item.observation || ''}-${item.originTable || ''}-${item.launchedByName || ''}`))).map(item => item.lineId || `${item.id}-${item.observation || ''}-${item.originTable || ''}-${item.launchedByName || ''}`)
      ),
