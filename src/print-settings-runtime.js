...
function openPrinterNameModal(current = '') {
  return new Promise(resolve => {
    document.querySelector('.receiptModalOverlay')?.remove()
    const overlay = document.createElement('div')
    overlay.className = 'receiptModalOverlay'
    overlay.innerHTML = `<div class="receiptModalBox" role="dialog" aria-modal="true"><div class="receiptModalIcon">🖨️</div><h3>${current ? 'Editar impressora' : 'Adicionar impressora'}</h3><p>Digite o nome da impressora.</p><input value="${escapeHtml(current)}" placeholder="Ex.: Impressora Cozinha"><div><button class="receiptModalCancel" type="button">Cancelar</button><button class="receiptModalSave" type="button">Salvar</button></div></div>`
    document.body.appendChild(overlay)
    const input = overlay.querySelector('input')
    const close = value => { overlay.remove(); resolve(value) }
    overlay.querySelector('.receiptModalCancel').addEventListener('click', () => close(null))
    overlay.querySelector('.receiptModalSave').addEventListener('click', () => close(input.value))
    overlay.addEventListener('click', e => { if (e.target === overlay) close(null) })
    setTimeout(() => input.focus(), 50)
  })
}

function openDeletePrinterModal(name) {
  return new Promise(resolve => {
    document.querySelector('.receiptModalOverlay')?.remove()
    const overlay = document.createElement('div')
    overlay.className = 'receiptModalOverlay'
    overlay.innerHTML = `<div class="receiptModalBox" role="dialog" aria-modal="true"><div class="receiptModalIcon">⚠️</div><h3>Excluir impressora?</h3><p>A impressora ${escapeHtml(name)} será removida das regras de impressão.</p><div><button class="receiptModalCancel" type="button">Cancelar</button><button class="receiptModalSave" type="button">Excluir</button></div></div>`
    document.body.appendChild(overlay)
    const close = value => { overlay.remove(); resolve(value) }
    overlay.querySelector('.receiptModalCancel').addEventListener('click', () => close(false))
    overlay.querySelector('.receiptModalSave').addEventListener('click', () => close(true))
...
function renderPrintPage(page, nav) {
  ...
  wrapper.innerHTML = `
    ...
    <section class="settingsPanel printRulesCard"><div class="printNumberTitle"><span>🧭</span><h2>2. Regras por setor</h2></div><div class="prRules"><div class="rulesFields"><label>Pedidos da cozinha saem em<select data-rule="cozinha">${opts(s, s.rules.cozinha)}</select></label><label>Pedidos do churrasco saem em<select data-rule="churrasco">${opts(s, s.rules.churrasco)}</select></label><label>Comanda do cliente sai em<select data-rule="cliente">${opts(s, s.rules.cliente)}</select></label></div><div class="currentRulesBox"><strong>Regras atuais</strong><ul><li>O sistema começa sem impressora cadastrada.</li><li>Cadastre somente as impressoras que existem.</li><li>Itens do bar ficam apenas na comanda do cliente.</li></ul></div></div></section>
...
function bindPrint(page, nav) {
  ...
  panel.querySelector('[data-add-printer]')?.addEventListener('click', async () => { const n = await openPrinterNameModal(''); if (!n?.trim()) return; const s = readPrint(); const p = { id: String(Date.now()), name: n.trim() }; const next = clean({ ...s, printers: [...s.printers, p] }); if (!next.rules.cozinha) next.rules.cozinha = p.id; if (!next.rules.cliente) next.rules.cliente = p.id; writePrint(next); renderPrintPage(page, nav); showToast('Impressora adicionada.') })
  ...
  panel.querySelectorAll('[data-act]').forEach(btn => btn.addEventListener('click', async () => { const id = btn.closest('[data-id]')?.dataset.id; const s = readPrint(); const p = s.printers.find(x => x.id === id); if (!p) return; if (btn.dataset.act === 'test') return showToast(`Teste preparado para ${p.name}.`); if (btn.dataset.act === 'edit') { const n = await openPrinterNameModal(p.name); if (!n?.trim()) return; writePrint({ ...s, printers: s.printers.map(x => x.id === id ? { ...x, name: n.trim() } : x) }); showToast('Nome da impressora atualizado.') } else { const ok = await openDeletePrinterModal(p.name); if (!ok) return; writePrint(clean({ ...s, printers: s.printers.filter(x => x.id !== id) })); showToast('Impressora removida.') } renderPrintPage(page, nav) }))
