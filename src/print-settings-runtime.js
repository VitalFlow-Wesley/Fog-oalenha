...
function readPrint() {
  try { return { ...emptyState, ...(JSON.parse(localStorage.getItem(PKEY) || 'null') || {}), rules: { ...emptyState.rules, ...((JSON.parse(localStorage.getItem(PKEY) || 'null') || {}).rules || {}) } } } catch { return emptyState }
}
function writePrint(v) { localStorage.setItem(PKEY, JSON.stringify(v)) }
function nameOf(s, id) { return s.printers.find(p => p.id === id)?.name || '' }
...
function renderPrintPage(page, nav) {
  ...
  Array.from(page.children).forEach(child => {
    if (child === nav || child.classList.contains('settingsTopHeader')) return
  ...
  page.insertAdjacentHTML('beforeend', `
    <div class="printRuntimePanel" data-print-runtime>
...
function restoreNative(page) { page.querySelector('[data-print-runtime]')?.remove(); Array.from(page.children).forEach(c => { if (!c.classList.contains('settingsTopHeader') && !c.classList.contains('settingsTabs')) c.style.display = '' }) }
function enhancePrint() { const page = document.querySelector('.settingsPremiumPage'); const nav = page?.querySelector('.settingsTabs'); const btn = nav && Array.from(nav.querySelectorAll('button')).find(b => b.textContent.includes('Impressão')); if (!page || !nav || !btn || btn.dataset.runtimePrint) return; btn.dataset.runtimePrint = '1'; btn.addEventListener('click', e => { e.preventDefault(); setTimeout(() => renderPrintPage(page, nav), 0) }); nav.querySelectorAll('button').forEach(b => { if (b !== btn) b.addEventListener('click', () => restoreNative(page)) }) }

const st = document.createElement('style'); st.textContent = '.printRuntimePanel{display:grid;grid-template-columns:1fr 1fr;gap:18px}.printRuntimePanel .printFooterBar{grid-column:1/-1}.prTable{border:1px solid #efd9bd;border-radius:16px;overflow:hidden}.prHead,.prRow{display:grid;grid-template-columns:1fr 1.2fr .7fr 1.1fr;gap:10px;align-items:center;padding:13px}.prHead{background:#fff0d7;font-weight:800}.prRow{border-top:1px solid #f1dfc8}.prRow span{display:flex;gap:8px}.prRow button{border:1px solid #e9d0b3;border-radius:10px;background:#fffaf2;padding:8px 10px;font-weight:800}.prRow .danger{color:#ef4444;background:#fff1ee}.prEmpty{padding:24px;text-align:center;color:#8a6b58}.prRules{display:grid;grid-template-columns:1fr 1fr;gap:16px}.prRules label{display:flex;flex-direction:column;gap:8px;font-weight:800}.prRules select{height:52px;border:1px solid #e7d0b3;border-radius:12px;padding:0 14px;background:#fffaf5}.prRules small{color:#8a6b58}.prRules .currentRulesBox{grid-row:1/5;grid-column:2;background:#eff9ea;border:1px solid #d6eccd;border-radius:16px;padding:18px}@media(max-width:1100px){.printRuntimePanel,.prRules{grid-template-columns:1fr}.prRules .currentRulesBox{grid-row:auto;grid-column:auto}.prHead,.prRow{grid-template-columns:1fr}}'; document.head.appendChild(st)
new MutationObserver(enhancePrint).observe(document.body,{childList:true,subtree:true}); window.addEventListener('DOMContentLoaded',enhancePrint); setTimeout(enhancePrint,300)
