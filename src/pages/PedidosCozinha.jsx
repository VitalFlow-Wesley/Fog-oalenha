...
import { enqueuePrintJob, fetchPrintJob } from '../services/printQueueApi.js'
  ...
  const tableOrders = tables
    ...
    .map(table => {
      ...
      return {
        ...
        printJobIds: [...new Set(items.map(item => item.lastPrintJobId).filter(Boolean))],
      }
    })
...
export default function PedidosCozinha({ tables, reservations = [], currentUser, settings }) {
  ...
  async function reprintOrder(order) {
    const originalId = order.printJobIds?.at(-1)
    if (!originalId) {
      alert('Não há uma comanda original registrada para reimprimir.')
      return
    }
    const original = await fetchPrintJob(originalId)
    if (!original) {
      alert('Não foi possível localizar a comanda original.')
      return
    }
    await enqueuePrintJob({
      ...
      waiterName: currentUser?.name || currentUser?.username || original.waiterName,
