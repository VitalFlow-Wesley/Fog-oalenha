...
function PasswordField({ label, value, onChange, visible, onToggle, placeholder, autoComplete = 'new-password' }) {
  return <label className="passwordSettingField"><span>{label}</span><div className="passwordInputWrap"><input type={visible ? 'text' : 'password'} value={value} placeholder={placeholder} onChange={onChange} autoComplete={autoComplete} /><button type="button" onClick={onToggle} aria-label="Mostrar ou ocultar senha">{visible ? <EyeOff size={16} /> : <Eye size={16} />}</button></div></label>
}
...
export default function Usuarios({ users, setUsers, tables, setTables, currentUser, settings, setSettings }) {
  ...
  return <div className="page settingsPremiumPage compactSettingsPage refinedSettingsPage">
    ...
    {editingUser && <div className="authModalOverlay">
      <form className="authModal settingsEditModal" onSubmit={saveEditedUser} autoComplete="off">
        ...
        <label><span>Login</span><input value={editingUser.username || ''} onChange={event => setEditingUser(prev => ({ ...prev, username: event.target.value }))} autoComplete="off" /></label>
        ...
        <label><span>Nova senha (opcional)</span><input type="password" value={editingUser.password || ''} onChange={event => setEditingUser(prev => ({ ...prev, password: event.target.value }))} placeholder="Deixe em branco para manter a atual" autoComplete="new-password" /></label>
