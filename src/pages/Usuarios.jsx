...
export default function Usuarios({ users, setUsers, tables, setTables, currentUser, settings, setSettings }) {
  ...
  function requestDeleteUser(user) {
    ...
    if (String(user.id) === String(currentUser?.id)) {
      setSystemMessage('Não é possível excluir o usuário conectado.')
      setTimeout(() => setSystemMessage(''), 2500)
      return
    }
  ...
  function confirmDeleteUser() {
    ...
    setSystemMessage('Acesso excluído com sucesso.')
    setTimeout(() => setSystemMessage(''), 2500)
  }
  ...
  // --- CARREGAMENTO DINÂMICO PARA EVITAR FALHA NO COMPILADOR ---
  ...
  function saveTextEdit(event) {
    ...
    if (textEditModal.type === 'table') {
      ...
      setSystemMessage('Nome da mesa atualizado.')
    }
    if (textEditModal.type === 'printer') {
      updatePrinterName(textEditModal.id, value)
      setSystemMessage('Nome da impressora atualizado.')
    }
    setTextEditModal(null)
    setTimeout(() => setSystemMessage(''), 2500)
  }
  ...
  function changeCancelPassword(e) {
    ...
    if (cancelForm.newPassword !== cancelForm.confirmPassword) return setCancelMessage('A confiormação da nova senha não confere.')
  ...
  return <div className="page settingsPremiumPage compactSettingsPage refinedSettingsPage">
    ...
    {deletingUser && <div className="authModalOverlay">
      <div className="authModal settingsDeleteModal" role="dialog" aria-modal="true">
        ...
        <div className="deleteWarningBox"><AlertTriangle size={20} /><div><strong>Essa ação remove o login do sistema.</strong><span>O usuário não poderá mais acessar o app depois da exclusão.</span></div></div>
