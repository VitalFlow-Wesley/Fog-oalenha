...
// Subcomponentes auxiliares mantidos isolados
...
export default function Usuarios({ users, setUsers, tables, setTables, currentUser, settings, setSettings }) {
  ...
  // --- REFACTOR 10: HELPER ENCAPSULADO DE NOTIFICAÇÕES ---
  ...
  // --- REFACTOR 3 & 11: MOTOR DE COMUNICAÇÃO ESC/POS ISOLADO ---
  async function executeThermalPrint(targetPrinterName, logLabel) {
    try {
      ...
      // --- REFACTOR 7: DESIGN DE CUPOM INDUSTRIAL ---
  ...
  // --- REFACTOR 12: MODAL COMPATÍVEL COM ESCUTA ASSÍNCRONA ---
  async function addPrinter() {
    try {
      ...
      console.error("Aviso de pareamento inicial em segundo plano ignorado:", erro);
    }
  ...
  // Exibição do modal de alteração de nome
  ...
  function restoreDefaults() { const defaults = normalizeSettings({ printers: basePrinters }); setSystemForm(defaults); showMessage('Padrões restaurados. Clique em salvar configurações para confirmar.') }
  
  // --- REFACTOR 1: LEITURA DINÂMICA REAL DOS ELEMENTOS DA DA TABELA ---
