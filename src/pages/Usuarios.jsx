...
export default function Usuarios({ users, setUsers, tables, setTables, currentUser, settings, setSettings }) {
  ...
  function editTableName(id) { const table = tableConfigs.find(item => item.id === id); const nextName = window.prompt('Nome exibido da mesa:', table?.displayName || ''); if (!nextName) return; setTableConfigs(prev => prev.map(item => item.id === id ? { ...item, displayName: nextName } : item)) }
