...
export default function Usuarios({ users, setUsers, tables, setTables, currentUser, settings, setSettings }) {
  ...
  // --- REFACTOR 10: HELPER PARA REDUZIR REPETIÇÃO DE SETTIMEOUT/MESSAGES ---
  ...
  // --- REFACTOR 3 & 11: ENGINE CENTRALIZADA DE IMPRESSÃO VIA CDN ---
  async function executeThermalPrint(targetPrinterName, logLabel) {
    try {
      const qz = window.qz;
      if (!qz) throw new Error("A ponte de hardware QZ Tray não está carregada.");

      if (!qz.websocket.isActive()) {
        await qz.websocket.connect();
      }

      // Procura o hardware dinâmico mapeado no Windows do caixa
      ...
      // --- REFACTOR 7: DESIGN DE CUPOM PROFISSIONAL EM PRODUÇÃO ---
      const receiptPayload = [
        '\x1B' + '\x40',          // Reset da impressora
        '\x1B' + '\x61' + '\x31', // Centralizar texto
        '================================\n',
        '         FOGAO A LENHA          \n',
        '       TESTE DE IMPRESSAO       \n',
        '================================\n',
        ...
        '--------------------------------\n',
        '\x1B' + '\x61' + '\x31', // Centralizar novamente
        'Status:\n',
        '[ OK - COMUNICACAO ATIVA ]\n',
        '================================\n',
        '\n\n\n\n',               
        '\x1D' + '\x56' + '\x41' + '\x00' // Comando ESC/POS de guilhotina/corte
      ];

      await qz.print(config, receiptPayload);
      return true;
    } catch (error) {
      console.error("Falha na execução do barramento de impressão:", error);
      alert(`Falha ao disparar barramento térmico: ${error.message || error}`);
      return false;
    }
  }

  // Mapeamento padrão do seu fluxo atual enquanto mantemos o modal original ativo
  async function addPrinter() {
    // Roda o teste na impressora padrão de comunicação antes de gerar o ID
    await executeThermalPrint("POS-80", "Nova Impressora Mapeada");
  ...
  // --- REFACTOR 1: CORREÇÃO DINÂMICA DO BOTÃO "TESTAR" DA TABELA ---
  async function testPrinter(name) {
    // Alvo dinâmico real baseado na string salva no input, com fallback seguro para POS-80
