...
function normalizeSettings(settings) {
  return {
    ...
    printers: settings?.printers || [
      { id: 'printer1', label: 'Impressora 1', name: settings?.printerKitchen || 'Impressora 1' },
      { id: 'printer2', label: 'Impressora 2', name: settings?.printerBar || 'Impressora 2' },
      { id: 'printer3', label: 'Impressora 3', name: settings?.printerCashier || 'Impressora 3' },
      { id: 'printer4', label: 'Impressora 4', name: 'Impressora 4' }
    ],
    activePrinterId: settings?.activePrinterId || 'printer1'
  }
}
...
export default function Usuarios({ users, setUsers, tables, setTables, currentUser, settings, setSettings }) {
  ...
  const settingsCards = [
    ...
    { icon: Printer, title: 'Impressoras', text: 'Cadastre até 4 impressoras e escolha qual será usada nos pedidos.' },
  ]
            ...
            <div className="settingsUsersList">
              {users.map(user => (
                <div className="settingsUserCard" key={user.id}>
                  ...
                  <div><strong>{user.name}</strong><span>Login: {user.username}</span><small>{roleLabel[user.role]}</small></div>
                  {canManage && user.id !== currentUser?.id && <button className="iconDanger" onClick={() => removeUser(user.id)} title="Remover usuário"><Trash2 size={18} /></button>}
                </div>
              ))}
            </div>
          </section>
      ...
      {activeTab === 'sistema' && (
        <div className="systemSettingsStack">
          <section className="settingsPanel wideSettingsPanel">
            ...
            <form className="systemSettingsForm printerSlotsForm" onSubmit={saveSystemSettings}>
              <label><span>Nome do estabelecimento</span><input value={systemForm.establishmentName} onChange={event => setSystemForm({ ...systemForm, establishmentName: event.target.value })} placeholder="Nome do restaurante" /></label>

              {systemForm.printers.map(printer => (
                <label key={printer.id}>
                  <span>{printer.label}</span>
                  <input value={printer.name} onChange={event => updatePrinterName(printer.id, event.target.value)} placeholder={`Nome da ${printer.label.toLowerCase()}`} />
                </label>
              ))}

              <label>
                <span>Impressora ativa para pedidos</span>
                <select value={systemForm.activePrinterId} onChange={event => setSystemForm({ ...systemForm, activePrinterId: event.target.value })}>
                  {systemForm.printers.map(printer => (
                    <option key={printer.id} value={printer.id}>{printer.name || printer.label}</option>
                  ))}
                </select>
              </label>

              <div className="activePrinterPreview">
                <Printer size={20} />
                <div>
                  <strong>Impressora ativa</strong>
                  <span>{activePrinter?.name || activePrinter?.label || 'Nenhuma impressora selecionada'}</span>
                </div>
              </div>

              <div className="printChecks">
                <label><input type="checkbox" checked={systemForm.printKitchenItems} onChange={event => setSystemForm({ ...systemForm, printKitchenItems: event.target.checked })} /> Imprimir cozinha/churrasco/sucos</label>
                <label><input type="checkbox" checked={systemForm.printBarItems} onChange={event => setSystemForm({ ...systemForm, printBarItems: event.target.checked })} /> Imprimir itens do bar também</label>
              </div>
