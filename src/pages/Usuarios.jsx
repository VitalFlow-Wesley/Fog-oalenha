...
export default function Usuarios({ users, setUsers, tables, setTables, currentUser, settings, setSettings }) {
  ...
  function addPrinter() { setSystemForm(prev => { const nextNumber = prev.printers.length + 1; return { ...prev, printers: [...prev.printers, { id: `printer-${Date.now()}`, label: `Impressora ${nextNumber}`, name: `Impressora ${nextNumber}` }] } }) }
  function removePrinter(id) { setSystemForm(prev => { if (prev.printers.length <= 1) return prev; const printers = prev.printers.filter(p => p.id !== id).map((p, i) => ({ ...p, label: `Impressora ${i + 1}` })); const fallback = printers[0].id; return { ...prev, printers, kitchenPrinterId: prev.kitchenPrinterId === id ? fallback : prev.kitchenPrinterId, cashierPrinterId: prev.cashierPrinterId === id ? fallback : prev.cashierPrinterId, grillPrinterId: prev.grillPrinterId === id ? fallback : prev.grillPrinterId, juicePrinterId: prev.juicePrinterId === id ? fallback : prev.juicePrinterId } }) }
  ...
  function saveSystem(e) { e?.preventDefault?.(); if (!canChangeSensitive) return; const printers = systemForm.printers.map((p, i) => ({ ...p, label: `Impressora ${i + 1}`, name: p.name.trim() || `Impressora ${i + 1}` })); setSettings(prev => ({ ...prev, ...systemForm, printers })); setSystemForm(prev => ({ ...prev, printers })); setSystemMessage('Configurações salvas com sucesso.'); setTimeout(() => setSystemMessage(''), 2500) }
  ...
  return <div className="page settingsPremiumPage compactSettingsPage refinedSettingsPage">
    ...
    {textEditModal && <div className="authModalOverlay">
      <form className="authModal settingsTextEditModal" onSubmit={saveTextEdit}>
        <div className="drawerHeader"><div><span className="eyebrow">{textEditModal.title}</span><h2>{textEditModal.subtitle}</h2></div><button type="button" className="iconBtn" onClick={() => setTextEditModal(null)}><X size={22} /></button></div>
        ...
        <div className="actionsRow"><button className="secondaryBtn" type="button" onClick={() => setTextEditModal(null)}>Cancelar</button><button className="primaryBtn" type="submit">Salvar</button></div>
      </form>
    </div>}
