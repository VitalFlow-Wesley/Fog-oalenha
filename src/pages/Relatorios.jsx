...
function buildReport(tables = []) {
  ...
  const topProducts = groupBy(items, item => item.name || 'Produto').slice(0, 8)
  ...
  const sectors = ['Cozinha', 'Churrasco', 'Sucos', 'Bar'].map(name => {
    ...
    return {
      ...
      total: sectorItems.reduce((sum, item) => sum + getItemTotal(item), 0),
    }
  })
  ...
  const topTable = salesByTable[0]
  const topProduct = topProducts[0]
  const topWaiter = waiterRanking[0]
  ...
  return { salesTables, sentTables, closedTables, items, total, ordersQty, ticket, topProducts, categories, sectors, salesByTable, waiterRanking, topWaiter, topTable, topProduct, hours }
}
...
export default function Relatorios({ tables }) {
  ...
  const summaryCards = [
    ...
    { title: 'Produto mais vendido', value: report.topProduct?.name || '-', detail: report.topProduct ? `${report.topProduct.qty} unidades` : 'Sem vendas', icon: PackageCheck, tone: 'orange' },
  ]
    ...
    <div className="page reportsPremiumPage completeReportPage">
      <div className="reportsHeader">
        <div>
          ...
          <p>{mode === 'completo' ? 'Análises detalhadas de vendas, pedidos, mesas, produtos e desempenho.' : 'Acompanhe resultados, produtos, faturamento e desempenho da operação.'}</p>
        </div>

        <div className="reportsActions noPrint">
          <div className="reportTabs">
            <button className={mode === 'simples' ? 'active' : ''} onClick={() => setMode('simples')} type="button">Relatório simples</button>
            <button className={mode === 'completo' ? 'active' : ''} onClick={() => setMode('completo')} type="button">Relatório completo</button>
          </div>
          <button className="reportActionBtn" type="button"><CalendarDays size={16} /> {period}</button>
      ...
      {mode === 'simples' ? (
        <>
          ...
          <div className="reportsMainGrid">
            <section className="reportPanel productsPanel"><div className="reportPanelHeader"><div><BarChart3 size={24} /><h2>Produtos mais lançados</h2></div><div className="periodTabs noPrint">{['Hoje', 'Semana', 'Mês'].map(item => <button type="button" key={item} className={period === item ? 'active' : ''} onClick={() => setPeriod(item)}>{item}</button>)}</div></div><div className="premiumReportTable"><div className="premiumReportRow head"><span>Produto</span><span>Qtd</span><span>Total</span></div>{report.topProducts.slice(0, 5).map(item => <div className="premiumReportRow" key={item.name}><span>{item.name}</span><span>{item.qty}</span><span>{money(item.total)}</span></div>)}</div><div className="liveDataPill"><CheckCircle2 size={15} /> Dados atualizados em tempo real</div></section>
            <aside className="reportPanel reportResumePanel"><div className="reportPanelHeader compact"><div><ClipboardList size={24} /><h2>Resumo do relatório</h2></div></div><div className="reportResumeList"><div><span><TrendingUp size={18} /> Faturamento do dia</span><strong>{money(report.total)}</strong></div><div><span><PackageCheck size={18} /> Pedidos lançados</span><strong>{report.ordersQty}</strong></div><div><span><ChefHat size={18} /> Mesas atendidas</span><strong>{report.salesTables.length}</strong></div><div><span><Martini size={18} /> Ticket médio</span><strong>{money(report.ticket)}</strong></div></div><div className="quickActions noPrint"><h3><Zap size={18} /> Ações rápidas</h3><div><button type="button" onClick={handlePrint}><Printer size={17} /> Imprimir relatório</button><button type="button" onClick={handleExportPdf}><Download size={17} /> Gerar PDF</button></div></div><div className="readyMessage"><CheckCircle2 size={17} /> Tudo pronto para conferência e fechamento.</div></aside>
          </div>
          <section className="completePreview noPrint"><div><ReceiptText size={23} /><strong>Relatório completo</strong><span>Inclui vendas, mesas, itens, desempenho e fechamento.</span></div><button type="button" onClick={() => setMode('completo')}>Visualizar prévia ›</button></section>
        </>
      ) : (
        <>
          ...
          <div className="completeReportLayout">
            ...
            <section className="reportPanel indicatorsPanel"><h2>Indicadores gerais</h2><div className="indicatorRows"><p><span>Total de comandas abertas</span><strong>{report.salesTables.length}</strong></p><p><span>Total de comandas fechadas</span><strong>{report.closedTables.length}</strong></p><p><span>Total de itens lançados</span><strong>{report.ordersQty}</strong></p><p><span>Ticket médio por mesa</span><strong>{money(report.ticket)}</strong></p><p><span>Média de itens por mesa</span><strong>{report.salesTables.length ? (report.ordersQty / report.salesTables.length).toFixed(1).replace('.', ',') : '0,0'}</strong></p></div></section>
            <section className="reportPanel generalSummaryPanel"><h2>Resumo geral</h2><div className="indicatorRows"><p><span>Faturamento total</span><strong>{money(report.total)}</strong></p><p><span>Mesa destaque</span><strong>{report.topTable ? `Mesa ${report.topTable.number} · ${money(report.topTable.total)}` : '-'}</strong></p><p><span>Produto destaque</span><strong>{report.topProduct ? `${report.topProduct.name} · ${report.topProduct.qty} un.` : '-'}</strong></p><p><span>Garçom destaque</span><strong>{report.topWaiter ? `${report.topWaiter.name} · ${report.topWaiter.tables} mesas · ${money(report.topWaiter.total)}` : '-'}</strong></p></div><div className="readyMessage"><CheckCircle2 size={17} /> Relatório gerado com os dados atuais das comandas.</div></section>
          </div>
        </>
      )}
    </div>
