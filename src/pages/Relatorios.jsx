...
export default function Relatorios({ tables = [], settings }) {
  ...
  const summaryCards = [
    ...
    { title: 'Comandas distintas atendidas', value: report.attendedTables, detail: 'Mesas diferentes com movimento no período', icon: Table2, tone: 'green' },
    { title: 'Ticket médio', value: money(report.ticket), detail: 'Por comanda distinta', icon: ClipboardList, tone: 'gold' },
    ...
    <div className="page reportsPremiumPage completeReportPage">
      <div className="reportsHeader">
        ...
        <div className="reportsActions noPrint">
          ...
          {mode === 'completo' && <label className="reportClosedToggle"><input type="checkbox" checked={includeClosedTables} onChange={event => setIncludeClosedTables(event.target.checked)} /> Incluir fechamentos do dia</label>}
        </div>
      </div>
      ...
      {mode === 'fechamentos' ? (
        ...
        <>
          ...
          <div className="completeReportLayout">
            ...
            <section className="reportPanel indicatorsPanel"><h2>Indicadores gerais</h2><div className="indicatorRows"><p><span>Comandas distintas com movimento</span><strong>{report.attendedTables}</strong></p><p><span>Fechamentos realizados</span><strong>{closedTablesForDate.length}</strong></p><p><span>Total de itens vendidos</span><strong>{report.ordersQty}</strong></p><p><span>Ticket médio por comanda</span><strong>{money(report.ticket)}</strong></p><p><span>Média de itens por comanda</span><strong>{report.attendedTables ? (report.ordersQty / report.attendedTables).toFixed(1).replace('.', ',') : '0,0'}</strong></p></div></section>
