...
function formatMoney(value) { return `R$ ${Number(value || 0).toFixed(2).replace('.', ',')}` }
...
export default function Mesas({ tables, setTables, users, currentUser, settings }) {
  ...
  function splitTables() {
    ...
    setTables(prev => prev.map(t => {
      ...
      if (table.mergedTableIds.includes(t.id)) { const previous = t.previousMergeState || t; return { ...previous, mergedTo: undefined, mergedToNumber: undefined, previousMergeState: undefined } }
      return t
    }))
  ...
  return <div className="page restaurantTablesPage">
    ...
    {table && <div className="drawerOverlay commandOverlay"><aside className="drawer commandDrawer">
      ...
      <header className="commandHeader"><div><span className="commandEyebrow">COMANDA ABERTA</span><div className="commandTitleRow"><h2>{tableLabel}</h2><span><Users size={19} /> {table.guests || 0} pessoas</span></div></div><div className="commandActions"><button className="commandBtn" onClick={joinTable} disabled={!joinTargetId}><Link2 size={18} /> Juntar mesas</button>{table.mergedTableIds?.length > 0 && <button className="commandBtn" onClick={splitTables}><Split size={18} /> Separar</button>}<button className="commandBtn" onClick={sendKitchen}><ChefHat size={18} /> Enviar para cozinha</button><button className="commandBtn" onClick={requestBill}><ReceiptText size={18} /> Solicitar conta</button><button className="commandBtn commandDanger" onClick={closeTable}>Fechar mesa</button></div></header>

      <div className="commandMainGrid"><section className="commandPanel"><h3>Itens da comanda</h3><div className="commandItemsList">{table.items.length === 0 && <p className="empty">Nenhum item lançado ainda.</p>}{table.items.map((item, index) => <div className="commandItem" key={`${item.id}-${index}-${item.observation}-${item.originTable || ''}`}><div className="commandItemInfo"><strong>{item.name}</strong><span>{item.localSaida === 'bar' ? 'Bar' : 'Cozinha / churrasqueiro'}</span><small>• {item.imprimeCozinha ? 'Sai no pedido da cozinha' : 'Sai na comanda do cliente'}</small>{item.originTable && <small>Origem: Mesa {item.originTable}</small>}{item.observation && <small>Obs.: {item.observation}</small>}</div><div className="commandItemControls"><div className="qtyStepper"><button onClick={() => changeQty(item, -1)}><Minus size={14} /></button><b>{item.qty}</b><button onClick={() => changeQty(item, 1)}><Plus size={14} /></button></div><button className="removeItemBtn" onClick={() => askCancelItem(item)}><Trash2 size={16} /></button><strong>{formatMoney(item.price * item.qty)}</strong></div></div>)}</div><div className="commandMergeBox"><select value={joinTargetId} onChange={e => setJoinTargetId(e.target.value)}><option value="">Escolha uma mesa para juntar</option>{mergeTargets.map(target => <option key={target.id} value={target.id}>Mesa {target.number} - {target.status === 'livre' ? 'Livre' : 'Ocupada'}</option>)}</select></div><div className="commandTotalCard"><div><span>Total da mesa</span><small>{totalItems} itens</small></div><strong>{formatMoney(total)}</strong></div></section>
