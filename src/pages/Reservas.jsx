...
function reservationHistoryLabel(reservation) {
  if (reservation.status === 'cancelada') return `Cancelada · ${reservation.closeReason || 'Sem motivo informado'}`
  if (reservation.status === 'nao_compareceu') return `Cliente não compareceu · ${reservation.closeReason || 'Sem motivo informado'}`
  if (reservation.status === 'transferida') return `Atendida · transferida para a mesa ${reservation.activatedTableNumber || '-'}`
  if (reservation.status === 'conta_solicitada') return `Atendida · conta solicitada (${reservation.activatedTableNumber || 'retirada'})`
  return 'Encerrada'
}
...
export default function Reservas({ reservations = [], setReservations, tables = [], currentUser, onActivate, onPersistReservations }) {
  ...
  const [sendingKitchenId, setSendingKitchenId] = useState('')
  ...
  async function sendKitchen(reservation) {
    ...
    if (!items.length) return setMessage('Esta reserva não possui itens configurados para preparo na cozinha.')
    setSendingKitchenId(reservation.id)
    try {
      const queued = await enqueuePrintJob({
        type: 'kitchen', kind: 'reservation', title: 'RESERVA PARA PREPARO',
        tableNumber: reservation.type === 'retirada' ? 'RETIRADA' : 'RESERVA',
        customerName: reservation.customerName, peopleCount: reservation.peopleCount,
        waiterName: currentUser?.name || currentUser?.username || 'Atendente', items,
  ...
  return <div className="reservasPage">
    ...
    {canManage && history.length > 0 && <section className="reservationPanel reservationHistory"><div className="panelHeading"><ClipboardList size={20} /><h2>Histórico de reservas</h2></div>{history.slice(0, 12).map(reservation => <div className="reservationHistoryRow" key={reservation.id}><strong>{reservation.customerName}</strong><span>{reservationHistoryLabel(reservation)}</span><small>{reservation.activatedBy || reservation.closedBy || '-'}</small></div>)}</section>}
  </div>
}
